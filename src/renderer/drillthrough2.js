...
Calendar.renderer.drillthrough2 = function(spec){
	...
	me.draw = function(){
		...
		var display = function(display){
			calendar._renderer = display.renderer;
		...
		calendar.eventManager.on("label:week:click", function(d){
			...
			me.current_display = {
				...
				, args: [d.getFullYear(), calendar.time.getWeek(d)]
			}
			display(me.current_display);
		});
		...
		calendar.eventManager.on("label:day:click", function(d){
			...
			me.current_display = {
				renderer : new Calendar.renderer.day()
				, args: [d.getFullYear(), calendar.time.getWeek(d), calendar.time.getDay(d)]
			}
			display(me.current_display);
		});
